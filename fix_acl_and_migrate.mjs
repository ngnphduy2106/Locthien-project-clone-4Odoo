...
async function grantAccess() {
    console.log('\n🔐 Granting access rights for custom models...');
    ...
    for (const modelName of models) {
        ...
        try {
            ...
            console.log(`  ✅ ${modelName}: Full CRUD granted`);
        } catch (e) {
...
async function migrateImportTickets() {
    console.log('\n📦 Import Tickets → x_locthien_import_ticket...');
    ...
    for (const t of data) {
        ...
        } catch (e) { errs++; if (errs <= 3) console.log(`    ⚠️ ${e.message.substring(0, 80)}`); }
    }
    console.log(`  ✅ Created: ${ok}, Skipped: ${skip}, Errors: ${errs}`);
}

async function migrateExportTickets() {
    console.log('\n📦 Export Tickets → x_locthien_export_ticket...');
...
async function migrateAssignmentsAndMerged() {
    console.log('\n📦 Driver Assignments → x_locthien_driver_assignment...');
    ...
    // Import Assignments
    console.log('\n📦 Import Assignments → x_locthien_import_assignment...');
    ...
    // Merged Orders
    console.log('\n📦 Merged Orders → x_locthien_merged_order...');
...
async function main() {
    ...
    console.log('🔐 FIX ACL + MIGRATE REMAINING DATA');
    console.log('='.repeat(60));
